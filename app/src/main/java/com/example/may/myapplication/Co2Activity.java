...
import static com.example.may.myapplication.MyDatabaseHelper.CREATE_CO2;
import static com.example.may.myapplication.MyDatabaseHelper.CREATE_TH;
...
//        long last = now -1000*60*24;
//
//        Cursor cursor = db.rawQuery("select * from co2 where time < " + now + " and time > " +last, null);
...
//        cursor.close();
        ...
        dbHelper = new MyDatabaseHelper((Context) this, "data.db", null, 1);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        Cursor cursor = db.query("Co2",null,"time > ?",null,null,null,null);
        if(cursor.moveToFirst()){
            do{
                double content = cursor.getDouble(cursor.getColumnIndex("content"));
                long time = cursor.getLong(cursor.getColumnIndex("time"));
        ...
        if(c <= 350){
            tv_2.setText("二氧化碳浓度太低了");
        }else if(350 <= c & c <= 1000){
            tv_2.setText("CO₂含量适宜，空气清新,呼吸顺畅");
        }else if(1000<c & c <=2000){
            tv_2.setText("CO₂含量较高，会感觉空气浑浊,并开始觉得昏昏欲睡");
        }else if(2000<c & c <= 5000){
            tv_2.setText("CO₂含量偏高，会感觉头痛、嗜睡、呆滞、注意力无法集中、心跳加速、轻度恶心");
        }else if(5000<c){
            tv_2.setText("CO₂含量过高，可能导致严重缺氧,造成永久性脑损伤、昏迷、甚至死亡");
        }
